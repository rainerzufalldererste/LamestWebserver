        ...
        public class AVLNode : ISerializable
        {
            ...
            internal static bool FindRemoveKey(AVLNode node, AVLTree<TKey, TValue> tree, TKey key, ref int elementCount)
            {
                ...
                while (true)
                {
                    ...
                    {
                        AVLNode.RemoveNode(node, tree, ref elementCount);
            ...
            internal static void RemoveNode(AVLNode headNode, AVLTree<TKey, TValue> tree, ref int elementCount)
            {
                if (headNode.right == null && headNode.left == null) // no children
                {
                    if (headNode.head == null) // was the top node
                    {
                        tree.head = null;
                    }
                    else
                    {
                        if (headNode.isLeft)
                        {
                            headNode.head.left = null;
                            headNode.head._depthL = 0;
                        }
                        else
                        {
                            headNode.head.right = null;
                            headNode.head._depthR = 0;
                        }

                        AVLNode.balanceSelfBubbleUp(headNode.head, tree);
                    }
                }
                else if (headNode.right == null || headNode.left == null) // one child
                {
                    AVLNode child = headNode.right != null ? headNode.right : headNode.left;
                    ...
                    {
                        child.isLeft = headNode.isLeft;

                        if (headNode.isLeft)
                        {
                            headNode.head.left = child;
                            child.head = headNode.head;
                            headNode.head._depthL -= 1;
                        }
                        else
                        {
                            headNode.head.right = child;
                            child.head = headNode.head;
                            headNode.head._depthR -= 1;
                        }
                ...
                {
                    AVLNode child = headNode.right, childhead = headNode.head;
                    ...
                    if (childhead != headNode.head)
                    {
                        ...
                        child.right = headNode.right;
                    }

                    child.left = headNode.left;
